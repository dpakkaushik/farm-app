    ...
    <div className="h-full flex flex-col bg-[var(--c-bg)]">
      <div className="shrink-0 px-4 pt-4 pb-2 flex items-center justify-between">
        <div>
          ...
          <p className="text-xs text-[var(--c-muted)]">{active.length} active crop cycles</p>
        </div>
          ...
          return (
            <div key={cycle.id} className={`bg-[var(--c-nav)] rounded-2xl border p-4 ${isReady?'border-[#1D9E75]/50':isNear?'border-[#BA7517]/40':'border-[var(--c-border)]'}`}>
              <div className="flex items-start justify-between mb-3">
                ...
                <div className="text-right">
                  {isReady ? (
                    ...
                    <>
                      <p className="text-2xl font-bold text-[var(--c-text)]">{remaining}</p>
                      <p className="text-[10px] text-[var(--c-muted)]">days left</p>
                    </>
                  )}
              ...
              <button onClick={()=>openRecord(cycle)}
                className={`w-full py-2.5 text-xs font-bold rounded-xl border transition-colors ${isReady?'bg-[#1D9E75] text-[var(--c-text)] border-transparent':'bg-[var(--c-card)] text-[var(--c-sub)] border-[var(--c-border-md)] hover:bg-[var(--c-ghost)]'}`}>
            ...
            {harvested.map(h=>(
              <div key={h.id} className="bg-[var(--c-nav)] rounded-2xl border border-[var(--c-border)] p-4 opacity-70">
                <div className="flex items-center justify-between">
                  <div>
                    <p className="text-sm font-semibold text-[var(--c-text)]">{cropMaster.find(c=>c.id===h.cropId)?.name||h.cropId} — {h.plotLabel}</p>
        ...
        <div className="fixed inset-0 z-50 flex items-end" style={{background:'rgba(0,0,0,0.7)'}} onClick={()=>setModal(null)}>
          <div className="w-full bg-[var(--c-nav)] rounded-t-3xl p-5 max-h-[85vh] overflow-y-auto border-t border-[var(--c-border-md)]" onClick={e=>e.stopPropagation()}>
            <div className="flex items-center justify-between mb-4">
              ...
              <button onClick={()=>setModal(null)} className="text-[var(--c-muted)]"><X size={18}/></button>
            </div>
            <div className="space-y-3">
              ...
              <button onClick={confirmRecord} className="w-full py-3 bg-[#1D9E75] text-[var(--c-text)] text-sm font-bold rounded-xl">Confirm Harvest</button>
            </div>
          </div>
        </div>
      )}
      ...
      {modal==='newCycle'&&(
        <div className="fixed inset-0 z-50 flex items-end" style={{background:'rgba(0,0,0,0.7)'}} onClick={()=>setModal(null)}>
          <div className="w-full bg-[var(--c-nav)] rounded-t-3xl p-5 max-h-[85vh] overflow-y-auto border-t border-[var(--c-border-md)]" onClick={e=>e.stopPropagation()}>
            ...
            <div className="space-y-3">
              <div><label className="text-xs text-[var(--c-sub)] block mb-1">Plot / Field name</label>
              ...
              <div><label className="text-xs text-[var(--c-sub)] block mb-1">Acres</label>
              ...
              <button onClick={confirmNewCycle} className="w-full py-3 bg-[#1D9E75] text-[var(--c-text)] text-sm font-bold rounded-xl">Start Cycle</button>
            </div>
          </div>
        </div>
      )}

      {toast&&<div className="fixed bottom-24 left-4 right-4 px-4 py-3 rounded-2xl text-sm font-medium text-[var(--c-text)] shadow-xl z-50 flex items-center gap-2 bg-[#1D9E75]"><CheckCircle2 size={16}/>{toast}</div>}
      <style>{`.finput{width:100%;background:var(--c-input);border:1px solid var(--c-border-md);border-radius:12px;padding:10px 14px;color:var(--c-text);font-size:14px;outline:none;}.finput:focus{border-color:#1D9E75;}.pulse{animation:pulse-ring 1.8s ease-in-out infinite;}@keyframes pulse-ring{0%,100%{opacity:1}50%{opacity:0.4}}`}</style>
    </div>
  )
}
