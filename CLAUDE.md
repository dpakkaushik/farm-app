...
the fold. **Add Plot is a sage pill directly under the tools button**, at his correction
(*"take add plot UP"* — the first cut put it bottom-right, opposite the legend). It sits **LAST
in that column and that ordering is load-bearing**: the coordinate and overlay panels anchor at
`top-3` and open only while the tools are expanded, so keeping the pill beneath them is what
stops a panel opening on top of it. The column needs `items-end` — flex's default `stretch`
would blow the 36px tool buttons up to the pill's width. **It is a DOOR, not a second
...
history, not the current rule. **The bottom nav lost its end gaps, same round.** *"at navigation bar there is a space in left
side when user clicks over the first nav tab … i dont want that gap."* Cause: `justify-around`
banks **half a gap at each end**, so the active pill floated off the bar's edge whenever the
first or last tab was selected. Now `justify-between` — every spare pixel goes between the
tabs — and `px-1` matches `py-1` so the inset is the same 4px on all four sides. Verified in a
